/**
 * fiveChainAI vs tempoFastAI の対戦評価 + コンボ分布 + 連鎖長ヒストグラム + 得点ペース/効率。
 *
 * 「人間戦略を直接ルール化した目的志向 AI（溜めて1ターン5連鎖を撃ち、それ以外では発火しない）が、
 * 現状最強 tempoFastAI に対し (1) 大連鎖を多く出すか、(2) self-play で勝てるか」を測る。
 *
 *   候補 = fiveChainAI を 1 席（rotate）、baseline = tempoFastAI を 3 席。
 *   候補席の勝率 + Wilson 95% CI（公平基準 0.25）、発火サイズ分布、1ターンの発火回数ヒストグラム
 *   （連鎖長分布）、N点到達ターン、手番あたり得点を同一対局から比較する。
 *
 * 注意: fiveChain は独自探索（nodeLimit）で動き timeBudget/lookahead を使わないため、baseline の
 *   tempoFast(--base-la,--budget) とは計算量が一致しない。勝率は「戦略が champion に勝てるか」を見る指標。
 *
 * 使い方:
 *   npx tsx ai/scripts/bench-fivechain.ts --games 48 --seed 75001 --base-la 0
 *   npx tsx ai/scripts/bench-fivechain.ts --games 24 --seed 75001 --base-la 1
 *
 *   --fire-target N   点火する最小連鎖（既定5）
 *   --node-limit N    fiveChain の探索ノード上限（既定60000）
 *   --base-la N       baseline tempoFast の lookaheadTurns（既定0。1で実体champion）
 *   --budget N        baseline tempoFast の timeBudgetMs（既定400）
 *   --opts '<json>'   FiveChainOptions の上書き
 */
import { setupGame } from '../../src/game/setup';
import { stepGame } from '../../src/game/reducer';
import type { Action, GameState } from '../../src/game/types';
import {
  decideAction as decideFive,
  type FiveChainOptions,
} from '../../src/ai/fiveChainAI';
...
function main(): void {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (k === '--fire-target') fireTarget = parseIntArg('--fire-target', argv[++i]);
    else if (k === '--node-limit') nodeLimit = parseIntArg('--node-limit', argv[++i]);
    else if (k === '--opts') optsJson = argv[++i];
    else throw new Error(`unknown arg: ${k}`);
  }

  const fiveOpts: FiveChainOptions = {
    fireTarget,
    nodeLimit,
    ...JSON.parse(optsJson),
  };

  const candidate: Decider = (s, p) => decideFive(s, p, undefined, fiveOpts);
  const baseline: Decider = (s, p) =>
    decideTempoFast(s, p, undefined, { timeBudgetMs: budget, lookaheadTurns: baseLA });

  console.error(
    `[bench-fivechain] cand=fiveChain(fireTarget=${fireTarget}, nodeLimit=${nodeLimit}) ` +
  ...
  console.log(`\n=== bench-adaptive 結果 ===`);
  console.log(
    `強さ: fiveChain 席 勝率 ${(winRate * 100).toFixed(1)}% (CI ${(ci.low * 100).toFixed(1)}-${(ci.high * 100).toFixed(1)}%) vs 公平25%  [${candWins}/${games}]`
  );
  ...
  console.log(`\nコンボサイズ別（手番100あたり）:`);
  console.log(
    `  fiveChain 3:${per100(cand, 3)}  4:${per100(cand, 4)}  5:${per100(cand, 5)}  6+:${sixPlus(cand)}`
  );
  console.log(
  ...
  console.log(`  fiveChain ${multiPct(cand)}%  (${cand.multiFireTurns}/${cand.playerTurns} ターン)`);
  ...
  console.log(
    `\n手番あたり得点（効率）: fiveChain ${(cand.scoreSum / Math.max(1, cand.turns)).toFixed(3)}   tempoFast ${(base.scoreSum / Math.max(1, base.turns)).toFixed(3)}`
  );
  console.log(`  参考: 人間 2.81 / AI 1.74（人間棋譜分析）`);
  console.log(
    `平均スコア: fiveChain席 ${(cand.scoreSum / games).toFixed(2)}点  tempoFast席 ${(base.scoreSum / (games * 3)).toFixed(2)}点  平均終了ターン ${(endTurnSum / games).toFixed(1)}  未完了 ${unfinished}局`
  );
