/**
 * Gen-3-O 用 joint 2D grid search: MCTS の `uctC × iterations` を網羅的に評価する。
 *
 * Gen-3-L (uctC grid, iter=400 固定) で uctC=2.0、
 * Gen-3-N (iterations grid, uctC=2.0 固定) で iter=400 が単独最適と確認できたが、
 * これは coordinate descent 解にすぎない。
 * 「単独で悪い uctC でも iter とのペアで補える」組合せがないか joint で検証する。
 *
 * Usage:
 *   npx tsx ai/scripts/grid-joint-uct-iter.ts \
 *     --games 100 --seed 1001 \
 *     --uct 1.7,2.0,2.4 --iter 200,400,800 \
 *     --json
 *
 * Options:
 *   --games <n>    1 候補あたりの局数 (default: 100)
 *   --seed <n>     base seed (default: 1001)
 *   --uct <list>   uctC 候補のカンマ区切り (default: 1.7,2.0,2.4)
 *   --iter <list>  iterations 候補のカンマ区切り (default: 200,400,800)
...
function parseArgs(): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case '--iter': {
        const raw = argv[++i];
        if (raw === undefined) throw new Error('--iter requires a value');
        const parts = raw.split(',').map((s) => Number(s.trim()));
        if (parts.some((v) => !Number.isFinite(v) || !Number.isInteger(v) || v <= 0)) {
          throw new Error('--iter must contain positive integers');
        }
        out.iterGrid = parts;
        break;
      }
...
function makeDecider(
  ...
  switch (strategy) {
    ...
    case 'mcts':
      return (s, pid) => decideMcts(s, pid, undefined, { uctC, iterations });
    default:
...
function playGame(
  ...
  const deciders = strategies.map((s) => makeDecider(s, uctC, iterations));
  let steps = 0;
...
function main(): void {
  ...
  for (const uctC of args.uctGrid) {
    for (const iterations of args.iterGrid) {
      ...
      process.stderr.write(
        `[grid] uctC=${uctC.toFixed(4).padStart(7)} iter=${iterations.toString().padStart(4)} running ${args.games} games... `
      );
      ...
      for (let g = 0; g < args.games; g++) {
        ...
        const r = playGame(seed, seats, uctC, iterations);
        stat.games += 1;
  ...
  if (args.json) {
    const out = {
      ...
      best: {
        uctC: sorted[0].uctC,
        iterations: sorted[0].iterations,
        wins: sorted[0].wins,
      },
    };
  ...
  for (const s of sorted) {
    ...
    console.log(
      `uctC=${s.uctC.toFixed(4).padStart(7)} iter=${s.iterations.toString().padStart(4)} ` +
  ...
  console.log(
    `\nbest: uctC=${sorted[0].uctC.toFixed(4)}, iter=${sorted[0].iterations} ` +
