...
- **強さ（公平25%）**: **fresh 事前登録テスト 34.13%（CI 30.92-37.48）＝旧既定 tempoChain（3席）に有意勝ち**・同条件 P=0.5 対照 28.25% を +5.9pt・**L2 逸脱テスト（P 軸）で self-play 不動点も確認**。無予算逸脱者は +6.9pt 勝つ＝**時間予算の残コスト ~7pt**（回収経路は実分布対応 C2・LA1。「予算を外す」は無予算 max 106 分で不可と実測済み）
...
| GRM（目標到達確率最大化法。第3路線＝学習も手書き評価も使わない確率プランナー）| **配信中（現ブラウザ既定）**。P\*=0.45（fresh 34.13%・L2 不動点確認・2026-06-12）。ゼロ損失×2 で劣化率 7-8%・p50 1ms。残る伸びしろ＝予算の残コスト ~7pt（無予算逸脱者 +6.9pt で実証）で、回収経路は**実分布対応 C2**（一様 i.i.d. 版は対戦不採用＝分布ずれが本丸と確定）と **LA1**（h 葉＋深さ1展開、tstar 実証→meteo 実験中）。q テーブルは不成立確定 | スキル無し。仕様 `ai/REACHABILITY.md`・目的 `ai/OBJECTIVE.md`・台帳 `ai/SPEED-PLAN.md`/`ai/TSTAR-DEPS.md` |
