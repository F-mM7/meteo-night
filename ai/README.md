...
| GRM（目標到達確率最大化法。第3路線＝学習も手書き評価も使わない確率プランナー）| **配信中（現ブラウザ既定）**。P\*=0.45（fresh 34.13%・L2 不動点確認・2026-06-12）。**K=6 も L2 不動点として確証**（K 軸第2回 2026-06-14。K=8 fresh n=800 で 26.13%＝parity）。ゼロ損失高速化で劣化率 7-8%・p50 1ms（連鎖直実装・memo キー bitpack に加え、2026-06-15 で内側 q ソルバの **memo キー単一パス＋resolveCombos の Map 撤去＝~30-35% 高速化・判断完全不変**）。残る伸びしろ＝予算の残コスト ~7pt（無予算逸脱者 +6.9pt で実証）で、回収経路は**実分布対応 C2**（一様 i.i.d. 版は対戦不採用＝分布ずれが本丸と確定）と **LA1**（h 葉＋深さ1展開、tstar 実証→meteo 実験中）。残る L2 対象＝ギフト配り族・deck15（速度ブロック中）。q テーブルは不成立確定。**読み合い（相手進捗の先読み race-read）も parity 見送り**（160局25.0%・発動1.4%・計算量は無コスト、2026-06-15。深い相手読みは予算崩壊＋設計原理に反す＝GRM の伸びしろでない）。**ゼロ損失の定数倍高速化は残差を劣化↓方向に縮めるが、残差 ~7pt の本質は厳密 q-DP コスト自体＝本格回収は C2 か非ゼロ損失近似が要る** | スキル無し。仕様 `ai/REACHABILITY.md`・目的 `ai/OBJECTIVE.md`・台帳 `ai/SPEED-PLAN.md`/`ai/TSTAR-DEPS.md` |
